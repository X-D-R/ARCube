...
def detect_pose(p_feat, sift_3d): #height weight from model
...
def registration_transform_3d_corners(rvecs, tvec, cameraMatrix, distCoeffs): #height weight from model
    pts = np.float32(model.object_corners_3d)
    #pts = np.float32([[0, 0, 0], [0, h - 1, 0], [w - 1, h - 1, 0], [w - 1, 0, 0]]) #3d corners
...
class frame_by_registration:
    ...
    def find_good_sift(self, shot, model, h, w, img1):

        '''dst, features, s_d = detect_features(self, shot, model, h, w, img1)
        #s_d = registration_find_3d_sifts(ob)'''
        MIN_MATCH_COUNT = 10
        sift = cv.SIFT_create()
    ...
    def track_features_sift(self, previous_frame, current_frame, feat):
    ...
    def draw_frame(self, frame, sift_3d, sift_2d, cameraMatrix, distCoeffs, model, h, w, p_feat):
        #if detect_pose(sift_2d, sift_3d) != False:
        print(len(p_feat), len(sift_3d))
        if len(p_feat) > 3 and len(p_feat) == len(sift_3d):
            valid, rvec, tvec = cv.solvePnP(sift_3d, p_feat, cameraMatrix, distCoeffs)
            rvecs = cv.Rodrigues(rvec)[0]
...
if __name__ == "__main__":
    ...
    img_ = 'reference.jpg'   #load reference photo
    ...
    pts_3d, feat = tracker.find_good_sift(previous_frame, model, h, w, img1)
    ...
    while True:
        ...
        good_new, good_old, p_feat = tracker.track_features_sift(previous_frame, frame, feat)
